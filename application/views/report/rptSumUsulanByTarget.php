...
if ($err) {
	...
	$valReportTitle="<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
			<tr>
				<td colspan=\"3\" style=\"text-align:left; text-transform:uppercase; font-size:11px; font-weight:bold;font-family:Arial, Helvetica, sans-serif;\">SUMMARY REPORT REQUEST BY TARGET</td>
			</tr>
	...
	if (strlen($userName)>0 or $userName<>"") {
			$valQuery="
				select '1' as group_total, count(a.request_id) as jumlah_usulan,target_name, sum(a.umum_unit) jumlah_unit
					from tp_request2 a 
					inner join tm_target_group d on a.umum_target_id=d.target_id 
					where  d.status in (1) ".$addQuery." 
...
//				order by group_total,propinsi_name asc,kabupaten_kota_name asc, umum_tgl_surat
	...
	$valQueryFilter="";
	$valArrRptFieldStructFieldName="group_total,target_name,jumlah_usulan,jumlah_unit";
	$valArrRptFieldStructFieldType="string,string,sumint,sumint";
	...
	if (strlen($userName)>0 or $userName<>"") {
			...
			$valRptColumnHeader="
					<tr class=\"rowHeader\">
						<td class=\"headerLeftU\" style=\"border-left:1px solid;border-top:1px solid;text-align:center;\" >Target/Peruntukan</td>
			...
			$valArrTempColDet1="
				<tr class=\"rowDetail\" >
						<td style=\"border-left:1px solid;text-align:center;\" class=\"detailLeftU\" >#target_name#</td>
