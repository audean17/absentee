...
if ($err) {
	...
	$valReportTitle="<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
			<tr>
				<td colspan=\"3\" style=\"text-align:left; text-transform:uppercase; font-size:11px; font-weight:bold;font-family:Arial, Helvetica, sans-serif;\">SUMMARY REPORT REQUEST BY PROVINSI</td>
			</tr>
	...
	if (strlen($userName)>0 or $userName<>"") {
			$valQuery="
				select '1' as group_total, count(a.request_id) as jumlah_usulan, b.propinsi_name, sum(a.umum_unit) jumlah_unit
					from tp_request2 a 
					inner join tm_propinsi b on a.umum_propinsi_id = b.propinsi_id 
					where  b.status in (1) ".$addQuery." 
					group by propinsi_name
...
//				order by group_total,propinsi_name asc,kabupaten_kota_name asc, umum_tgl_surat
	...
	$valQueryFilter="";
	$valArrRptFieldStructFieldName="group_total,propinsi_name,jumlah_usulan,jumlah_unit";
