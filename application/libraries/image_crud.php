...
class image_CRUD {
	...
	protected $ci = null;
	protected $thumbnail_prefix = 'thumb_s_';
    ...
    protected function _get_delete_url($value)
    {
    	...
    	return site_url('admin/'.$rsegments_array[1].'/'.$rsegments_array[2].'/delete_file/'.$value);
    }
	...
	protected function _create_thumbnail($image_path, $thumbnail_path)
	{
        ...
        protected function _create_thumbnail_big($image_path, $thumbnail_path)
	{
		$this->image_moo
			->load($image_path)
			->resize_crop(240,200)
        ...
        protected function _watermark($image_path) {
            $ci = &get_instance();
            $ci->load->library('image_lib');
            $rConfig = array(
                'wm_type' => 'overlay',
                'wm_overlay_path' => APPPATH.'../assets/img/watermark.jpg',
                'wm_opacity' => 50,
                'wm_x_transp' => 0,
                'wm_y_transp' => 0,
	...
	protected function getState()
	{
		...
		{
			$upload_url = site_url('admin/'.$rsegments_array[1].'/'.$rsegments_array[2].'/upload_file/'.$rsegments_array[3]);
			$ajax_list_url  = site_url('admin/'.$rsegments_array[1].'/'.$rsegments_array[2].'/'.$rsegments_array[3].'/ajax_list');
			$ordering_url  = site_url('admin/'.$rsegments_array[1].'/'.$rsegments_array[2].'/ordering');
			$insert_title_url  = site_url('admin/'.$rsegments_array[1].'/'.$rsegments_array[2].'/insert_title');
		...
		{
			$upload_url = site_url('admin/'.$rsegments_array[1].'/'.$rsegments_array[2].'/upload_file');
			$ajax_list_url  = site_url('admin/'.$rsegments_array[1].'/'.$rsegments_array[2].'/ajax_list');
			$ordering_url  = site_url('admin/'.$rsegments_array[1].'/'.$rsegments_array[2].'/ordering');
			$insert_title_url  = site_url('admin/'.$rsegments_array[1].'/'.$rsegments_array[2].'/insert_title');
		...
		if(!empty($state_info))
		{
			switch ($state_info->name) {
				...
				case 'upload_file':
					...
					if ($file_name !== false) {
                                                $this->_watermark( $this->image_path.'/'.$file_name );
						$this->_create_thumbnail_big( $this->image_path.'/'.$file_name , $this->image_path.'/thumb_'.$file_name );
